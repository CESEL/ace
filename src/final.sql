...
--Lucene
--update clt set pqn = simple, reason = 'type: 3, but used in more than two posts', trust = 0 where kind = 'type' and trust = 3 and simple in (select simple from clt where kind = 'type' and trust = 0 group by simple having count(*) > 1) and simple !~ E'^(Lucene\.Net)$';
